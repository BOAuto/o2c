...
export const Route = createFileRoute("/_layout/clients")({ component: CompaniesPage })
...
export function CompaniesPage() {
  ...
  const { data: domains } = useQuery({
    queryKey: ["companyDomains", selectedCompanyId],
    queryFn: () => ClientsApi.listCompanyDomains(selectedCompanyId),
    enabled: !!selectedCompanyId,
  })
  const { data: contracts } = useQuery({
    queryKey: ["rateContracts", selectedCompanyId],
    queryFn: () => ClientsApi.listRateContracts(selectedCompanyId || undefined),
  })
  ...
  const createOrUpdateCompany = useMutation({
    mutationFn: () => {
      ...
      return selectedCompanyId ? ClientsApi.updateCompany(selectedCompanyId, payload) : ClientsApi.createCompany(payload)
    },
    onSuccess: async (result) => {
      showSuccessToast(selectedCompanyId ? "Company updated" : "Company created")
      await queryClient.invalidateQueries({ queryKey: ["companies"] })
      const companyId = (result as { id?: string }).id
  ...
  const addDomain = useMutation({
    mutationFn: () => ClientsApi.createCompanyDomain(selectedCompanyId, domainPattern),
    onSuccess: () => {
      showSuccessToast("Domain added")
      setDomainPattern("")
      queryClient.invalidateQueries({ queryKey: ["companyDomains", selectedCompanyId] })
    },
    onError: handleError.bind(showErrorToast),
  })
  const deleteDomain = useMutation({
    mutationFn: (domainId: string) => ClientsApi.deleteCompanyDomain(domainId),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["companyDomains", selectedCompanyId] }),
    onError: handleError.bind(showErrorToast),
  })
  const assignRule = useMutation({
    mutationFn: (ruleId: string) => ClientsApi.createValidationAssignment({ company_id: selectedCompanyId, validation_rule_id: ruleId }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["validationAssignments", selectedCompanyId] }),
    onError: handleError.bind(showErrorToast),
  })
  ...
  const persistRows = async (incomingRows: ContractRow[]) => {
    ...
    for (const row of normalized) {
      ...
      if (!existing) {
        await ClientsApi.createRateContract({ company_id: selectedCompanyId, ...row })
      }
    }
    ...
    await queryClient.invalidateQueries({ queryKey: ["rateContracts", selectedCompanyId] })
  }
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">ERP-style split layout with wizard and global validation catalog.</p>
      </div>
        ...
        <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
          {pagedCompanies.map((company) => (
            <button
              type="button"
              key={company.id}
              className={`rounded-md border px-3 py-2 text-left ${selectedCompanyId === company.id ? "border-primary bg-muted/40" : ""}`}
              onClick={() => {
                setSelectedCompanyId(company.id)
                setCompanyName(company.name)
                setPaymentTerm(company.payment_term ? String(company.payment_term) : "")
                setAkaNames(company.aka_names?.join(", ") ?? "")
                setWizardStep(1)
              }}
            >
          ...
          {wizardStep === 2 && (
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={() => addDomain.mutate()} disabled={!selectedCompanyId || !domainPattern}>Add</Button>
              </div>
            ...
            <>
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button onClick={() => persistRows(rows)} disabled={!selectedCompanyId}>Import from rows</Button>
                  <Button variant="outline" onClick={exportCsv} disabled={!contractList.some((row) => row.is_active)}>Export CSV</Button>
                  <Button variant="outline" onClick={() => csvFileRef.current?.click()} disabled={!selectedCompanyId}>Import CSV</Button>
                  <input
                    ref={csvFileRef}
                    type="file"
                    accept=".csv,text/csv"
                ...
                {contractList.length ? (
                  contractList.map((contract) => (
                    <div key={contract.id} className="rounded-md border px-3 py-2 text-sm">
                      {contract.product_name} ({contract.sku}) • Rate {contract.agreed_rate} • GST{" "}
                      {contract.gst_rate}% • {contract.is_active ? "Active" : "Inactive"}
                    </div>
                  ))
                ) : (
                ...
                return (
                  <div key={rule.id} className="flex items-center justify-between rounded-md border px-3 py-2">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => assignment ? removeRuleAssignment.mutate(assignment.id) : assignRule.mutate(rule.id)}
                    >
      ...
      <Dialog open={isMissingDialogOpen} onOpenChange={setIsMissingDialogOpen}>
        <DialogContent className="sm:max-w-xl">
              ...
              <div key={contract.id} className="flex items-center gap-3 rounded-md border px-3 py-2 text-sm">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => {
                    setSelectedContractIdsForDeactivate((prev) =>
                      checked ? [...prev, contract.id] : prev.filter((id) => id !== contract.id),
                    )
                  }}
                />
          ...
          <DialogFooter>
            ...
            <Button
              onClick={async () => {
                for (const id of selectedContractIdsForDeactivate) {
                  await ClientsApi.updateRateContract(id, { is_active: false })
                }
                await queryClient.invalidateQueries({ queryKey: ["rateContracts", selectedCompanyId] })
